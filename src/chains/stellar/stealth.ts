...
/**
 * Computes the v2 view tag for a Stellar stealth announcement from public keys only.
 *
 * Unlike the legacy {@link computeViewTag} (which requires the ECDH shared secret),
 * the v2 tag hashes the ephemeral and viewing public keys directly. This lets senders
 * and recipients compute the same tag without exposing the shared secret.
 *
 * @param ephPubKey - 32-byte ephemeral ed25519 public key generated by the sender.
 * @param viewingPubKey - 32-byte recipient ed25519 viewing public key.
 * @returns Integer view tag in the range 0–255.
 */
export function computeAnnouncementViewTag(
  ...
  const input = new Uint8Array(VIEW_TAG_PREFIX.length + ephPubKey.length + viewingPubKey.length);
  input.set(VIEW_TAG_PREFIX);
  input.set(ephPubKey, VIEW_TAG_PREFIX.length);
  input.set(viewingPubKey, VIEW_TAG_PREFIX.length + ephPubKey.length);
  return sha256(input)[0];
}
