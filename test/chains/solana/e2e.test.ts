...
import { scanAnnouncements } from '../../../src/chains/solana/scan';
...
describe('e2e: full stealth payment flow on Solana', () => {
  test('derive → generate → scan → spend → verify', () => {
    ...
    expect(stealth.stealthAddress).toBeTruthy();
    ...
    const stealthPub = ed25519.ExtendedPoint.BASE.multiply(
      matched[0].stealthPrivateScalar,
    ).toRawBytes();
    expect(bytesToHex(stealthPub)).toBe(bytesToHex(matched[0].stealthPubKeyBytes));
