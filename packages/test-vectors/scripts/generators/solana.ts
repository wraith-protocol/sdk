...
export function generateSolanaVectors(seed: string, count: number): VectorSet {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const signature_sign = signSolanaTransaction(txHash, keys.spendingScalar, stealthPubKey);
  ...
  return {
    version: '1.0.0',
    chain: 'solana',
    description: 'Solana stealth address test vectors (ed25519, X25519 ECDH)',
    keyDerivation,
    stealthGeneration,
    scanMatch,
    signing,
